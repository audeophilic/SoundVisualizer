...
public:
    ...
    double estimateTempoOfSamples(const double *samples, int nsamples)
    {
        int i = 0;
    ...
    void process(const double *samples, int nsamples)
    {
        int n = 0;
        while (n < nsamples) {
...
MiniBPM::MiniBPM(double sampleRate) :
...
double
MiniBPM::estimateTempoOfSamples(const double *samples, int nsamples)
{
    return m_d->estimateTempoOfSamples(samples, nsamples);
}

void
MiniBPM::process(const double *samples, int nsamples)
{
