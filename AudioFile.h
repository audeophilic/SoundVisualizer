...
{
public:
    struct AudioFileErrorType { std::string msg; };
...
bool AudioFile<T>::load (std::string filePath)
{
    ...
    if (! file.good())
    {
        reportError ( "File doesn't exist or otherwise can't load file "  + filePath);
        return true;
    }
	...
	{
		reportError ("Couldn't read entire file: " + filePath);
		return false;
	}
    ...
    {
        reportError ("Audio file type not supported");
        return false;
    }
}
...
bool AudioFile<T>::decodeWaveFile (std::vector<uint8_t>& fileData)
{
    ...
    {
        reportError ("This doesn't seem to be a valid .WAV file");
        return false;
    }
    ...
    {
        reportError ("This .WAV file is encoded in a format that this library does not support at present");
        return false;
    }
    ...
    {
        reportError ("This WAV file seems to be an invalid number of channels (or corrupted?)");
        return false;
    }
    ...
    {
        reportError ("The header data in this WAV file seems to be inconsistent");
        return false;
    }
    ...
    {
        reportError ("This file has a bit depth that is not 8, 16, 24 or 32 bits");
        return false;
    }
    ...
    for (int i = 0; i < numSamples; i++)
    {
        for (int channel = 0; channel < numChannels; channel++)
        {
            ...
            {
                reportError ("Read file error as the metadata indicates more samples than there are in the file data");
                return false;
            }
...
bool AudioFile<T>::decodeAiffFile (std::vector<uint8_t>& fileData)
{
    ...
    {
        reportError ("This doesn't seem to be a valid AIFF file");
        return false;
    }
    ...
    if (sampleRate == 0)
    {
        reportError ("This AIFF file has an unsupported sample rate");
        return false;
    }
    ...
    {
        reportError ("This AIFF file seems to be neither mono nor stereo (perhaps multi-track, or corrupted?)");
        return false;
    }
    ...
    {
        reportError ("The metadatafor this file doesn't seem right");
        return false;
    }
...
bool AudioFile<T>::saveToWaveFile (std::string filePath)
{
    ...
    {
        reportError ("Couldn't save file to " + filePath);
        return false;
    }
...
{
    throw AudioFileErrorType{ errorMessage };
}
